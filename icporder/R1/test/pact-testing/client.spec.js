  ...
  // we want to test the function that is calling the external service.
    ...
    describe('and there are records in the sample data', () => {
      ...
      it('returns a list of records', done => {
        client.geticporder((error,response)=>{
    ...
    describe('and there is an record in the data with that ID', () => {
      before(() => provider.addInteraction({
        state: 'Has a record with ID '+expectedRecords[0]['orderId'],
        uponReceiving: 'a request for a record with ID '+expectedRecords[0]['orderId'],
        withRequest: {
          method: 'GET',
          path: BASE_PATH+'/'+expectedRecords[0]['orderId']
        },
        willRespondWith: {
          status: 200,
          headers: {
      ...
      it('returns the record', done => {
        client.geticporderById(expectedRecords[0]['orderId'],(error,response)=>{
          var parsedData = JSON.parse(response.body);
          expect(parsedData).to.have.deep.property('orderId', expectedRecords[0]['orderId']);
    ...
    describe('and there no records in the database', () => {
      ...
      it('returns a 404', done => {
        client.geticporderById('NonExisting',(error,response)=>{
      ...
      describe('and there is a record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['orderId'],
          uponReceiving: 'a request to update a record with ID '+expectedRecords[0]['orderId'],
          withRequest: {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json'
        ...
        it('returns the updated record', done => {
          client.puticporder(expectedRecords[0]['orderId'], expectedRecords[0], (error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('orderId', expectedRecords[0]['orderId']);
            done();
          })
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.puticporder('NonExisting', expectedRecords[0], (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
      ...
      describe('and the record abides by the required schema', () => {
        ...
        it('returns the newly added record', done => {
          client.posticporder(expectedRecords[0],(error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('orderId', expectedRecords[0]['orderId']);
            done();
          });
      ...
      describe('and the record is invalid with respect to the schema', () => {
        ...
        it('returns 400 error', done => {
          client.posticporder({ 'dummyKey': 'dummyValue' },(error,response)=>{
      ...
      delete semiRecord['orderId'];
      describe('and there is an record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['orderId'],
          uponReceiving: 'a request to semi-update a record with ID '+expectedRecords[0]['orderId'],
          withRequest: {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json'
        ...
        it('returns the updated record', done => {
          client.patchicporder(expectedRecords[0]['orderId'], semiRecord, (error,response)=>{
            var parsedData = JSON.parse(response.body);
            expect(parsedData).to.have.deep.property('orderId', expectedRecords[0]['orderId']);
            done();
          })
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.patchicporder('NonExisting', semiRecord, (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
      ...
      describe('and there is a record in the data with that ID', () => {
        before(() => provider.addInteraction({
          state: 'Has a record with ID '+expectedRecords[0]['orderId'],
          uponReceiving: 'a request to delete a record with ID '+expectedRecords[0]['orderId'],
          withRequest: {
            method: 'DELETE',
            path: BASE_PATH+'/'+expectedRecords[0]['orderId']
          },
        ...
        it('returns 204', done => {
          client.deleteicporder(expectedRecords[0]['orderId'], (error,response)=>{
      ...
      describe('and there no records in the database with that ID', () => {
        ...
        it('returns a 404', done => {
          client.deleteicporder('NonExisting', (error,response)=>{
            expect(response.statusCode).to.be.eql(404);
            done();
          })
        })
